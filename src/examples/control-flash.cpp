...
void disco(Flash &flash)
{
	srand(time(0));
	while (true)
	{
		flash.Ui(rand() % 64, rand() % 64, rand() % 64);
...
void temperature(Flash &flash)
{
	while (true)
	{
		auto temp = flash.Temperature();
...
void info(Flash &flash)
{
	auto version = flash.Version();
...
void power(Flash &flash)
{
	// flash.CurrentLevel(43);

	cout << "Current level: " << (int)flash.CurrentLevel() << '\n';
	cout << "Voltage level: " << (int)flash.VoltageLevel() << '\n';
	cout << "   Voltage in: " << flash.VoltageIn() << "V\n";
	cout << "  Voltage out: " << flash.VoltageOut() << "V\n";
}


// Display the timing configuration for continuous flash mode
void continuous(Flash &flash)
{
	// flash.Continuous({ 0x1000, 0x1000, 0x1000, 0x1000 });

	for (auto &f : flash.Continuous())
	{
		cout << String::format("%04x\t", f);
	}
	cout << "\n";
...
{
	map<string, function<void(Flash&)>> commands = {
		{ "find",			[](auto &f) {}},
		{ "list", 			[](auto &f) { list(); }},
		{ "off",			[](auto &f) { f.Ui(0, 0, 0); }},
		{ "on",				[](auto &f) { f.Ui(255, 0, 0); }},
		{ "info",			[](auto &f) { info(f); }},
		{ "temperature",	[](auto &f) { temperature(f); }},
		{ "disco",			[](auto &f) { disco(f); }},
		{ "group",			[](auto &f) { group(f); }},
		{ "power",			[](auto &f) { power(f); }}
	};
